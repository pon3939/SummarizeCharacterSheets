...
```bash
SummarizeCharacterSheets$ sam build --use-container
```
...
```bash
SummarizeCharacterSheets$ sam logs -n StockCheckerFunction --stack-name "summarizecharactersheets" --tail
```
...
```bash
SummarizeCharacterSheets$ pip install -r tests/requirements.txt --user
# unit test
SummarizeCharacterSheets$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
SummarizeCharacterSheets$ AWS_SAM_STACK_NAME="summarizecharactersheets" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "summarizecharactersheets"
```
